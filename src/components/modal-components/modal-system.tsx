...
import { Plus, X, ChevronDown, XIcon } from "lucide-react"
...
export default function ModalSystem({ open, setOpen }: ModalSystemProps) {
    ...
    const Navigation = () => (
        <div className="flex sm:flex-row flex-col items-center gap-2 sm:gap-4 sm:justify-start justify-center px-4">
            <div className="flex items-center gap-2">
                <button onClick={() => setActiveTab("profile")} className={`rounded-lg px-3 py-2 border transition-colors ${activeTab === "profile" ? "border-black bg-gray-50" : "border-gray-400 bg-transparent hover:border-gray-600"}`}>
                    Profile
                </button>
                <button onClick={() => setActiveTab("admin")} className={`rounded-lg px-3 py-2 border transition-colors ${activeTab === "admin" ? "border-black bg-gray-50" : "border-gray-400 bg-transparent hover:border-gray-600"}`}>
                    Admin
                </button>
            </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <button onClick={() => setActiveTab("subscription")} className={`rounded-lg px-3 py-2 border transition-colors ${activeTab === "subscription" ? "border-black bg-gray-50"
    ...
    const ManageGroup = () => (
        <div className="p-4">
            <h3 className="text-xl sm:text-2xl font-semibold mt-4">Manage Groups</h3>
            <div className="flex items-center gap-2 font-medium text-black text-lg sm:text-xl mt-5">
                You have created 0/1 groups
    ...
    const AddGroup = () => (
        <div className="p-4">
            ...
            <button
                onClick={() => groupName.trim() && setGroupsView("modal")}
    ...
    const ModalGroup = () => (
        <div className="p-4 w-full max-w-full">
            ...
            <button className="rounded-lg py-2 px-2.5 border border-gray-500 font-medium text-black mt-4 mb-6 text-sm sm:text-base">
                Add a Member
            </button>
            ...
            <div className="space-y-3">
                {members.map((member) => (
                    <div key={member.id} className="flex sm:grid sm:grid-cols-[40px_60px_1fr_1fr_1fr] sm:gap-4 items-center">
                        <button className="w-6 h-6 sm:w-8 sm:h-8 p-0 hover:bg-gray-100 flex-shrink-0 rounded-full">
            ...
            <button className="w-full lg:w-1/4 bg-black text-white hover:bg-gray-800 mt-6 py-2.5 sm:py-3 rounded-lg font-medium text-sm sm:text-base">
                Save
            </button>
        </div>
    )
            ...
            <div className='mt-10 flex flex-col mx-auto justify-center items-center max-w-md'>
                ...
                <div className='flex md:flex-row flex-col items-center justify-center lg:gap-10 gap-5'>
                    <div className="md:w-24 md:h-[70px] w-20 h-20 rounded-full mb-8 overflow-hidden transition-all duration-300 relative">
                        ...
                        <label htmlFor="profile-image-upload" className="absolute inset-0 cursor-pointer"style={{ opacity: 0 }}>
                        </label>
                    </div>
                    ...
                    <div className="grid grid-cols-6 gap-2 mb-10 w-full">
                        {profileImages.map((image, index) => (
                            <button
                                key={index}
                                className={`w-8 h-8 aspect-square rounded-full overflow-hidden transition-all duration-200 ${selectedImage === image
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="xl:max-w-5xl lg:max-w-4xl md:max-w-3xl sm:max-w-2xl max-w-[95vw] max-h-[95vh] overflow-y-auto p-4">
