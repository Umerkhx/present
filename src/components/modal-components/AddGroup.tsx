...
export default function AddGroup() {
  ...
  return (
    <div className="p-4">
      <h3 className="text-xl sm:text-2xl font-bold mt-4">Add a Group</h3>
      ...
      <div className="border-2 border-gray-300 rounded-md p-6 mt-7 mb-5 h-32 sm:h-48 flex flex-col items-center justify-center text-center hover:border-gray-400 transition-colors">
        <h4 className="font-bold text-base sm:text-lg">Upload your roster here</h4>
        <p className="text-sm font-medium text-gray-900 mb-4">CSV or Excel docs only (max {limits.maxMembersPerGroup} members)</p>
        <input type="file" accept=".csv,.xlsx,.xls" onChange={handleFileUpload} className="hidden" id="file-upload" />
        <label htmlFor="file-upload" className="bg-black text-white font-medium px-4 py-2 rounded-md cursor-pointer text-sm">
          Choose File
        </label>
      </div>
